...
try {
  const assetPath = resolve(distDir, "assets/monero-worker.js");
  const mainPath = resolve(distDir, "challenge-monero.js");
  const workerSource = await readFile(assetPath, "utf8");
  const prefix = `globalThis.__POUWER_EMBEDDED_MONERO_WORKER = ${JSON.stringify(workerSource)};\n`;
